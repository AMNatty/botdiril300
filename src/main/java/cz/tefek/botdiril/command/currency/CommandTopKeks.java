...
public class CommandTopKeks
{
    ...
    public static void show(CallObj co)
    {
        ...
        BotMain.sql.exec("SELECT us_userid, us_keks FROM " + UserInventory.TABLE_USER + " WHERE us_userid<>? ORDER BY us_keks DESC LIMIT " + LIMIT, stat ->
        {
            ...
            while (rs.next())
            {
                ...
                var row = String.format("%s with **%d** %s", usn, rs.getLong("us_keks"), Icons.KEK);
